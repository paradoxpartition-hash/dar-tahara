...
/*
 * PATCHED for Mautic 7.1.3 on PHP 8.3.
 *
 * Upstream bug: includeWithEvent() is typed `: string`, but Twig's
 * CoreExtension::include() returns a Twig\Markup object when the included
 * template is html-safe (e.g. the login page @MauticUser/Security/base.html.twig).
 * On PHP 8.3 that trips a TypeError and every page using the slim/login layout
 * 500s. Fix: cast both return values to (string). Remove this patch (and the
 * bind-mount in docker-compose.yml) once Mautic ships a version that fixes it.
 *
 * Original file extracted from mautic/mautic:7.1.3-apache.
 */
